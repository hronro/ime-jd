...
If the IME **delegates** punctuation to the engine, it doesn't have to do anything special — just route the actually-typed byte to `jd_press_key`. The full set of mapped keys is the union of the keys listed in `src/punctuation-marks/normal.txt` and `paired.txt`.
