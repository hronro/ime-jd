...
fn main() {
    ...
    //      stays a deliberate `0.0.0` placeholder. (The same string is also
    //      exported to the crate as JD_VERSION, see below.)
    //   3. The seal icon. RegisterProfile points pchIconFile at this DLL with
    //      uIconIndex 0, i.e. the first ICON resource — which is this one.
    //      jd.ico is generated by assets/icon/generate.sh.
    ...
    // Also hand the full version string to the crate (`env!("JD_VERSION")`),
    // so the update check compares the release feed against this build.
    println!("cargo:rustc-env=JD_VERSION={version}");
    ...
    // Copy app.manifest and jd.ico beside the generated .rc so their relative
    // references resolve regardless of the resource compiler's working
    // directory.
    fs::copy(
    ...
    fs::copy(manifest_dir.join("jd.ico"), out_dir.join("jd.ico"))
        .expect("copy jd.ico into OUT_DIR");
    ...
    println!("cargo:rerun-if-changed=jd.ico");
