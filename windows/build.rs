...
fn main() {
    ...
    // libjd itself is located and linked by the `jd` crate's build script
    // (bindings/rust/build.rs) — always statically on Windows, so unlike the
    // CLI there is no rpath to emit here.
